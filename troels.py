...
def create_db(tableOfTuples):
	...
	d.execute('CREATE TABLE documents (id, date, case_name, case_number, file_number, parties, ruling)')
			...
			if y == 1: 
				...
				for cell in row:
					...
					if x == 1: pass						
						...
						#url =  "http://www.icj-cij.org/" + filename

						case_number = int(filename.split("/")[-2])
						
						#int filename.split("/").rfind(".")
						...
						case_details = str(id), date, case_name, case_number, file_number, parties, ruling
